...
const chartItems: ChartItem[] = [
  // 基础图表
  { id: "bar", name: "柱状图", component: BarChart, category: "基础图表", icon: "📊", description: "展示分类数据对比" },
  { id: "line", name: "折线图", component: LineChart, category: "基础图表", icon: "📈", description: "展示数据趋势变化" },
  { id: "area", name: "面积图", component: AreaChart, category: "基础图表", icon: "📐", description: "强调数据量的累积" },
  ...
  { id: "radar", name: "雷达图", component: RadarChart, category: "数据对比", icon: "🎯", description: "多维度能力对比" },
  { id: "radial-bar", name: "径向柱状图", component: RadialBarChart, category: "数据对比", icon: "🌀", description: "环形柱状对比" },
  { id: "negative-bar", name: "正负条形图", component: NegativeBarChart, category: "数据对比", icon: "⚖️", description: "正负数据对比" },
  { id: "polar-bar", name: "极坐标柱状图", component: PolarBarChart, category: "数据对比", icon: "🎡", description: "环形数据对比" },
  { id: "parallel", name: "平行坐标图", component: ParallelChart, category: "数据对比", icon: "📏", description: "多维度数据分析" },
  // 数据分布
  { id: "histogram", name: "直方图", component: Histogram, category: "数据分布", icon: "📊", description: "展示数据分布频率" },
  { id: "box-plot", name: "箱线图", component: BoxPlot, category: "数据分布", icon: "📦", description: "展示数据离散程度" },
  { id: "scatter", name: "散点图", component: ScatterPlot, category: "数据分布", icon: "⚪", description: "展示数据相关性" },
  { id: "bubble", name: "气泡图", component: BubbleChart, category: "数据分布", icon: "🫧", description: "三维数据展示" },
  { id: "heatmap", name: "热力图", component: Heatmap, category: "数据分布", icon: "🔥", description: "矩阵数据密度展示" },
  { id: "treemap", name: "矩形树图", component: TreemapChart, category: "数据分布", icon: "🗂️", description: "层级占比分析" },
  // 趋势分析
  { id: "stacked-area", name: "堆叠面积图", component: StackedAreaChart, category: "趋势分析", icon: "📐", description: "多系列累积趋势" },
...
export const ChartsPage: Component = () => {
  const [selectedCategory, setSelectedCategory] = createSignal<ChartCategory>("基础图表");
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-muted-foreground">
                探索 {stats().total} 个数据可视化图表，涵盖 {stats().categories} 个分类
              </p>
            </div>
          </div>
        </div>
                    ...
                    {(category) => (
                      ...
                      >
                        <div class="flex items-center gap-3">
                          ...
                          <div class="flex-1">
                            <div class={`font-medium ${selectedCategory() === category.id ? "text-foreground" : ""}`}>
                              {category.name}
                            </div>
                          ...
                          {selectedCategory() === category.id && (
                            <div class={`w-1 h-8 rounded-full bg-gradient-to-b ${category.gradient}`}></div>
                          )}
                        </div>
                      </button>
                    )}
                ...
                <Show when={currentCategoryInfo()}>
                  {(categoryInfo) => (
                    <div class="flex items-center gap-4 mb-8">
                      <div class={`h-10 w-1 bg-gradient-to-b ${categoryInfo().gradient} rounded-full`}></div>
                      <div>
                        <div class="flex items-center gap-3">
                          <span class="text-2xl">{categoryInfo().icon}</span>
                          <h2 class="text-2xl font-bold">{categoryInfo().name}</h2>
                        </div>
                        <p class="text-muted-foreground mt-1">{categoryInfo().description}</p>
                      </div>
                    </div>
                  )}
                </Show>
