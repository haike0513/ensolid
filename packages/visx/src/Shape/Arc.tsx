...
import { arc as d3Arc, Arc as D3Arc } from 'd3-shape';
...
export const Arc: Component<ArcProps> = (props) => {
  ...
  const path = createMemo(() => {
    ...
    if (local.innerRadius !== undefined) arcGen.innerRadius(local.innerRadius as any);
    if (local.outerRadius !== undefined) arcGen.outerRadius(local.outerRadius as any);
    if (local.cornerRadius !== undefined) arcGen.cornerRadius(local.cornerRadius as any);
    if (local.startAngle !== undefined) arcGen.startAngle(local.startAngle as any);
